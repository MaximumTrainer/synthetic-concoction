...
public sealed class LlmOptions
{
    ...
    /// <summary>Retries of a retryable provider failure (rate limit, transport, timeout, 5xx) with exponential backoff. Zero disables.</summary>
    ...
    public static LlmOptions FromEnvironment(Func<string, string?> getEnvironmentVariable)
    {
        ...
        if (int.TryParse(Get("MAX_RETRIES"), out var maxRetries)) options.MaxRetries = maxRetries;
    ...
    public IReadOnlyList<string> Validate()
    {
        ...
        if (MaxRetries < 0) errors.Add($"{EnvPrefix}MAX_RETRIES must be zero (disabled) or positive.");
